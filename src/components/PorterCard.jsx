...
import { FaStar, FaRoute, FaWeightHanging } from 'react-icons/fa';
import Modal from './Modal';
import ImageGallery from './ImageGallery';

const PorterCard = ({ porter }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { name, photo, experience, maxWeight, rating, reviews, basePrice, routes, certifications, description } = porter;
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{maxWeight}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {routes.map((route, index) => (
                      <div key={index} className="flex items-center gap-2 bg-green-50 p-3 rounded-lg">
                        <FaRoute className="text-green-600 w-5 h-5" />
